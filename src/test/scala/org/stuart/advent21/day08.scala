...
import org.stuart.advent21.Signals.{SignalProcessor, DataPreProcessor}
import org.scalatest._
import com.typesafe.scalalogging.LazyLogging

class day08 extends FunSuite with Matchers with LazyLogging {

  test("The signal processor should correctly count possible unique outputs") {
    val signalProcessor = new SignalProcessor
    val outputSignals = new DataPreProcessor(day08.testData).outputSignals
    val result = outputSignals.map(signalProcessor.uniqueNums).sum
    result shouldBe day08.testExpectedPart1
  }

  test ("(produce submission for part 1)") {
    val signalProcessor = new SignalProcessor
    val outputSignals = new DataPreProcessor(day08.input).outputSignals
    val result = outputSignals.map(signalProcessor.uniqueNums).sum
    logger.info(s"result: $result")
  }

  test("The signal processor should correctly map signals to segments and sum the outputs") {
    val signalProcessor = new SignalProcessor
    val preProcessor = new DataPreProcessor(day08.testData)
    val inputSignals = preProcessor.inputSignals
    val outputSignals = preProcessor.outputSignals
    val result = signalProcessor.decodeAndSumAll(inputSignals, outputSignals)
    result shouldBe day08.testExpectedPart2
  }

  test ("(produce submission for part 2)") {
   val signalProcessor = new SignalProcessor
    val preProcessor = new DataPreProcessor(day08.input)
    val inputSignals = preProcessor.inputSignals
    val outputSignals = preProcessor.outputSignals
    val result = signalProcessor.decodeAndSumAll(inputSignals, outputSignals)
    logger.info(s"result: $result")
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce"""
  ...
  val testExpectedPart1 = 26
  val testExpectedPart2 = 61229
